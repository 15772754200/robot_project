...
def find_latest_log_file(ros_log_dir=None):
    ...
    ros_log_dir = ros_log_dir or str(run_log_root())
    if not os.path.exists(ros_log_dir):
        print(f"❌ ROS日志目录不存在: {ros_log_dir}")
        return None

    # 支持递归，因为每次启动都会在 run_logs/realsense 下创建独立子目录
    pattern = os.path.join(ros_log_dir, "**", "realsense_start_*.log")
...
def main():
    ...
    parser.add_argument(
        "-l", "--log-dir",
        help="日志根目录（默认 /home/niic/robot_ros2_deploy_4_10_humanoid/run_logs/realsense）"
    )
    parser.add_argument(
        "-f", "--file",
