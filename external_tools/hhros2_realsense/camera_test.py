...
def detect_running_camera_nodes():
    """检查是否已有 ROS2 相机节点在运行。"""
    try:
    ...
    for node in nodes:
        lower = node.lower()
        if "realsense" in lower or node in ("/camera", "/camera/camera"):
            hits.append(node)
    return hits
...
def test_realsense_device():
    ...
    if len(devices) == 0:
        ...
        running_nodes = detect_running_camera_nodes()
        if running_nodes:
            print("⚠️ 同时检测到以下 ROS2 相机节点正在运行：")
            for node in running_nodes:
                print(f"   - {node}")
            print("💡 直连硬件测试会直接访问相机底层设备。")
            print("   如果相机已经被 ROS2 驱动占用，直连测试可能失败。")
            print("   请先停止相机节点，再重新执行本测试。")
        sys.exit(1)
