...
同目录下需要存在：
- start.sh             : 启动 realsense2_camera (ros2 launch...)，并写 run_logs/camera/realsense_start_*.log
- camera_menu.py       : 运行时调参 + topic 选择/echo
- record.py            : 订阅图像话题，拍照/录像，写 run_logs/camera下的日志与媒体文件
- camera_log_read.py   : 打印最新（或指定）的 realsense_start_*.log
- camera_test.py       : pyrealsense2 直连测试（不依赖 ROS2）

建议：用普通用户运行本菜单脚本（避免 HOME 变为 /root 导致日志路径变化）。
"""
...
def action_read_startup_log(base: Path) -> None:
    ...
    log_dir = ""
    if not file_path:
        log_dir = input("指定 ROS 日志根目录（回车=默认 run_logs/camera）：").strip()
...
def action_quick_check(base: Path) -> None:
    ...
    if rc == 0:
        ...
        print("⚠️ ros2 命令不可用：请先运行 rosenv 进入本项目环境。")
