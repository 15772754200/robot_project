...
# === 根据实际情况修改你的相机节点名称 ===
# 对于 rs_launch.py 默认配置，你的节点名就是 /camera/camera （和你发的 topic 一致）
...
def ensure_camera_node():
    ...
    if CAMERA_NODE in nodes:
        print(f"[信息] 检测到相机节点: {CAMERA_NODE}")
    else:
        print(f"[警告] 在 node list 中没有找到 {CAMERA_NODE}")
        print("       请确认相机已经启动，或者修改脚本顶部的 CAMERA_NODE。")
...
def param_menu():
    ...
    while True:
        ...
        cur_val = get_param(CAMERA_NODE, param_name)
        ...
        set_param(CAMERA_NODE, param_name, value_str)
