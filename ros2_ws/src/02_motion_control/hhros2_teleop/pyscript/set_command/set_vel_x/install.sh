...
show_help() {
    ...
    echo -e "  ${GREEN}w / W${NC}    前进 (增加线速度X)"
    echo -e "  ${GREEN}s / S${NC}    后退 (减少线速度X)"
    echo -e "  ${GREEN}r / R${NC}    停止 (速度归零)"
    echo -e "  ${GREEN}c / C${NC}    显示当前速度"
    echo -e "  ${RED}x / X${NC}    退出程序"
    echo ""
    echo -e "${YELLOW}示例:${NC}"
    echo "  $0 -t /robot1/cmd_vel -l 1.5"
    echo -e "${GREEN}=========================================${NC}\n"
}
...
send_velocity() {
    ...
    # 限制线速度X范围
    if (( $(echo "$linear_x > $LINEAR_MAX" | bc -l) )); then
        linear_x=$LINEAR_MAX
    elif (( $(echo "$linear_x < -$LINEAR_MAX" | bc -l) )); then
        linear_x=-$LINEAR_MAX
    fi
    ...
    echo -e "${GREEN}✓ 发送速度: 前进/后退速度 = ${LINEAR_X} m/s${NC}"
}

# 显示当前状态
show_status() {
    echo -e "\n${CYAN}════════════════════════════════════════${NC}"
    echo -e "${CYAN}            前进后退控制状态${NC}"
    echo -e "${CYAN}════════════════════════════════════════${NC}"
    ...
    echo ""
    echo -e "当前前进/后退速度: ${GREEN}$LINEAR_X${NC} m/s"
    echo -e "方向: ${YELLOW}$(if (( $(echo "$LINEAR_X > 0" | bc -l) )); then echo "前进"; elif (( $(echo "$LINEAR_X < 0" | bc -l) )); then echo "后退"; else echo "停止"; fi)${NC}"
    echo -e "${CYAN}════════════════════════════════════════${NC}"
}
...
show_instructions() {
    ...
    echo -e "${YELLOW}[控制命令]${NC}"
    echo -e "  ${GREEN}W${NC}: 前进 (速度 +$STEP)"
    echo -e "  ${GREEN}S${NC}: 后退 (速度 -$STEP)"
...
input_speed() {
    ...
    read_line input_speed "请输入前进/后退速度 (m/s): "
    
    if [[ $input_speed =~ ^-?[0-9]+\.?[0-9]*$ ]]; then
        send_velocity $input_speed 0.0 0.0
    else
...
control_loop() {
    ...
    while true; do
        echo -e "\n${YELLOW}等待输入命令 (W:前进, S:后退, I:手动输入, R:停止, C:状态, X:退出)...${NC}"
        read_key key
        
        case $key in
            w|W)  # 前进
                LINEAR_X=$(echo "$LINEAR_X + $STEP" | bc)
                send_velocity $LINEAR_X 0.0 0.0
                ;;
            s|S)  # 后退
                LINEAR_X=$(echo "$LINEAR_X - $STEP" | bc)
                send_velocity $LINEAR_X 0.0 0.0
...
# 主程序
clear
echo -e "${GREEN}机器人前进后退控制脚本启动中...${NC}"
