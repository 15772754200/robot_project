...
show_help() {
    ...
    echo -e "  ${GREEN}1${NC}    切换到爬坡步态模式 (模式3)"
    ...
    if grep -Eq 'success[=:][[:space:]]*(true|True)' <<<"$response"; then
        ...
        case $mode in
            2)
                ...
                ;;
            3)
                echo -e "${GREEN}✓ 发送控制模式: 爬坡步态 (模式=$mode)${NC}"
                ;;
            *)
...
show_status() {
    ...
    case $CURRENT_MODE in
        2)
            ...
            ;;
        1)
            echo -e "状态: ${GREEN}爬坡步态模式${NC}"
            echo -e "描述: 机器人正在使用爬坡步态移动${NC}"
            ;;
        *)
...
# 切换到爬坡步态模式（完整的切换流程）
switch_to_climbing_gait() {
    echo -e "\n${CYAN}════════════════════════════════════════${NC}"
    echo -e "${CYAN}          切换到爬坡步态模式${NC}"
    echo -e "${CYAN}════════════════════════════════════════${NC}"
    echo -e "切换流程:"
    echo -e "  1. 先发布停止步态 (模式2)"
    echo -e "  2. 等待2秒"
    echo -e "  3. 再发布爬坡步态 (模式3)"
    echo -e "${CYAN}════════════════════════════════════════${NC}"
    ...
    # 步骤3: 发布爬坡步态 (模式3)
    echo -e "\n${YELLOW}步骤3: 发布爬坡步态命令...${NC}"
    send_control_mode 3 "爬坡步态模式"
    
    echo -e "\n${GREEN}✓ 切换流程完成！机器人已切换到爬坡步态模式${NC}"
}
...
show_instructions() {
    ...
    echo -e "  ${GREEN}模式3${NC}: 爬坡步态 - 机器人使用爬坡步态移动"
    echo ""
    echo -e "${YELLOW}[切换流程]${NC}"
    echo -e "  选择按键1时会执行完整切换流程:"
    echo -e "  1. 先发布模式2 (停止步态)"
    echo -e "  2. 等待2秒"
    echo -e "  3. 再发布模式3 (爬坡步态)"
    echo ""
...
control_loop() {
    ...
    while true; do
        echo -e "\n${YELLOW}等待输入命令 (1:爬坡步态, 2:停止步态, C:状态, X:退出)...${NC}"
        read_key key
...
# 主程序
clear
echo -e "${GREEN}机器人爬坡步态控制脚本启动中...${NC}"
