    ...
    if grep -Eq 'success[=:][[:space:]]*(true|True)' <<<"$response"; then
        case $mode in
            2)
                ...
                ;;
            3)
                echo -e "${GREEN}✓ 发送步态控制: 行走步态 (模式=$mode)${NC}"
                ;;
            *)
...
control_loop() {
    ...
    # 脚本启动后自动切换到行走步态
    echo -e "${GREEN}脚本启动，切换到行走步态模式...${NC}"
    send_gait_mode "$RUNNING_MODE" "切换到行走步态"
    
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}✓ 行走步态已启动，现在可以使用$INPUT_MODE模式控制机器人${NC}"
    else
        echo -e "${RED}✗ 启动行走步态失败，继续控制程序${NC}"
    fi
