...
# 发送爬坡步态停止命令的函数
send_stop_climbing_command() {
    local attempt=$1
    local total=$2
    
    echo -e "${YELLOW}发送第 $attempt/$total 次停止爬坡命令...${NC}"
    ...
    if grep -Eq 'success[=:][[:space:]]*(true|True)' <<<"$response"; then
        echo -e "  ${GREEN}✓ 成功发送停止爬坡命令${NC}"
        echo -e "  ${GREEN}  服务: $TOPIC${NC}"
        echo -e "  ${GREEN}  消息: 停止爬坡步态 (模式2)${NC}"
        return 0
    else
        echo -e "  ${RED}✗ 发送失败${NC}"
        return 1
...
send_multiple_stop_commands() {
    ...
    for i in $(seq 1 $MESSAGE_COUNT); do
        send_stop_climbing_command $i $MESSAGE_COUNT
    ...
    if [ $success -gt 0 ]; then
        echo -e "\n${GREEN}✓ 成功发送 $success/$MESSAGE_COUNT 次爬坡停止命令${NC}"
        return 0
    else
        echo -e "\n${RED}✗ 所有发送尝试都失败${NC}"
        return 1
...
show_instructions() {
    ...
    echo -e "  ${GREEN}1${NC}: 发送一次爬坡停止命令"
    echo -e "  ${GREEN}2${NC}: 发送多次爬坡停止命令"
...
control_loop() {
    ...
    # 脚本启动后自动发送一次停止命令
    echo -e "${GREEN}脚本启动，自动发送爬坡步态停止命令...${NC}"
    send_single_stop
