...
show_help() {
    ...
    echo ""
    echo -e "${YELLOW}功能:${NC}"
    echo "  启动后自动向指定话题发布停止跳跃控制消息 (模式2)"
    echo "  机器人将从跳跃状态切换到站立状态"
    echo ""
    echo -e "${YELLOW}示例:${NC}"
...
# 发送停止跳跃控制命令
send_stop_jump_command() {
    echo -e "${CYAN}════════════════════════════════════════${NC}"
    echo -e "${CYAN}         发布停止跳跃控制命令${NC}"
    echo -e "${CYAN}════════════════════════════════════════${NC}"
    echo -e "目标服务: ${YELLOW}$TOPIC${NC}"
    echo -e "控制模式: ${GREEN}停止跳跃/站立 (模式2)${NC}"
    echo -e "服务类型: ${GREEN}hhros2_interfaces/srv/SetControlMode${NC}"
    echo -e "数据值: ${GREEN}$MESSAGE_VALUE${NC}"
    echo -e "${CYAN}════════════════════════════════════════${NC}"
    
    # 发送 ROS2 消息
    echo -e "${YELLOW}发布停止跳跃控制命令...${NC}"
    
    local response
    response="$(ros2 service call "$TOPIC" \
        hhros2_interfaces/srv/SetControlMode \
        "{mode: $MESSAGE_VALUE}" 2>&1)"
    printf '%s\n' "$response"
    
    if grep -Eq 'success[=:][[:space:]]*(true|True)' <<<"$response"; then
        echo -e "${GREEN}✓ 成功发布停止跳跃控制命令${NC}"
        echo -e "${GREEN}✓ 服务: $TOPIC${NC}"
        echo -e "${GREEN}✓ 消息: 停止跳跃/站立 (模式2)${NC}"
    else
        echo -e "${RED}✗ 发布停止跳跃控制命令失败${NC}"
...
test_topic_existence() {
    ...
    if ros2 service list 2>/dev/null | grep -q "^${TOPIC}$"; then
        ...
        ros2 service list 2>/dev/null | grep -i "control\|gait\|mode\|cmd" | head -10 || echo "  (没有找到相关服务)"
        return 1
    fi
}
...
show_instructions() {
    ...
    echo ""
    echo -e "${YELLOW}[注意]${NC}"
    echo -e "  脚本启动后已自动发送停止跳跃命令"
    echo -e "  机器人将从跳跃状态切换到站立状态"
    echo -e "${GREEN}════════════════════════════════════════${NC}\n"
}
...
control_loop() {
    ...
    # 脚本启动后立即发送停止跳跃命令
    echo -e "${GREEN}脚本启动，自动发送停止跳跃控制命令...${NC}"
    send_stop_jump_command
    
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}✓ 停止跳跃命令已发送${NC}"
    else
        echo -e "${RED}✗ 发送停止跳跃命令失败，继续控制程序${NC}"
    fi
    ...
    while true; do
        ...
        case $key in
            ...
            t|T)  # 检查服务
                ...
                ;;
            x|X)  # 退出
                echo -e "${RED}退出停止跳跃控制程序${NC}"
                
                exit 0
                ;;
            *)
                if [ -n "$key" ]; then
                    echo -e "${YELLOW}无效按键，按 S 重新发送停止跳跃命令${NC}"
                fi
                ;;
        esac
    done
...
# 主程序
clear
echo -e "${GREEN}机器人停止跳跃控制脚本启动中...${NC}"
