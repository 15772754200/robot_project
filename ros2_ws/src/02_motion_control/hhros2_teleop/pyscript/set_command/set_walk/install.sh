...
show_help() {
    ...
    echo -e "  ${GREEN}1${NC}    切换到行走步态模式 (模式3)"
    ...
    if grep -Eq 'success[=:][[:space:]]*(true|True)' <<<"$response"; then
        ...
        case $mode in
            2)
                ...
                ;;
            3)
                echo -e "${GREEN}✓ 发送控制模式: 行走步态 (模式=$mode)${NC}"
                ;;
            *)
...
show_status() {
    ...
    case $CURRENT_MODE in
        2)
            ...
            ;;
        1)
            echo -e "状态: ${GREEN}行走步态模式${NC}"
            echo -e "描述: 机器人正在使用行走步态移动${NC}"
            ;;
        *)
...
# 切换到行走步态模式（完整的切换流程）
switch_to_walking_gait() {
    echo -e "\n${CYAN}════════════════════════════════════════${NC}"
    echo -e "${CYAN}          切换到行走步态模式${NC}"
    echo -e "${CYAN}════════════════════════════════════════${NC}"
    echo -e "切换流程:"
    echo -e "  1. 先发布停止步态 (模式2)"
    echo -e "  2. 等待2秒"
    echo -e "  3. 再发布行走步态 (模式3)"
    echo -e "${CYAN}════════════════════════════════════════${NC}"
    ...
    # 步骤3: 发布行走步态 (模式3)
    echo -e "\n${YELLOW}步骤3: 发布行走步态命令...${NC}"
    send_control_mode 3 "行走步态模式"
    
    echo -e "\n${GREEN}✓ 切换流程完成！机器人已切换到行走步态模式${NC}"
}
...
control_loop() {
    ...
    while true; do
        echo -e "\n${YELLOW}等待输入命令 (1:行走步态, 2:停止步态, C:状态, X:退出)...${NC}"
        read_key key
...
# 主程序
clear
echo -e "${GREEN}机器人行走步态控制脚本启动中...${NC}"
