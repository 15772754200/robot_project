#!/bin/bash
# 启动 RGB 相机脚本（由 enrgb 调用）
# 功能：启动 RealSense 彩色相机
...
if [ "$FOUND_TYPE" = "source" ] || [ "$FOUND_TYPE" = "incomplete" ]; then
    echo ""
    echo "⚠️  检测到 RealSense 需要编译（状态: $FOUND_TYPE）"
    echo "开始自动编译..."
...
# ---------- 检测 RealSense 设备 ----------
echo ""
echo "🔍 检测 RealSense 硬件设备..."

if ! command -v rs-enumerate-devices &> /dev/null; then
    echo "  📦 安装 librealsense2-utils..."
    sudo apt install -y librealsense2-utils
fi

DEVICE_FOUND=false
if command -v rs-enumerate-devices &> /dev/null; then
    DEVICE_INFO=$(rs-enumerate-devices 2>&1)
    if echo "$DEVICE_INFO" | grep -q "Device info"; then
        DEVICE_FOUND=true
        echo "✅ 检测到 RealSense 设备"
        echo "$DEVICE_INFO" | head -20
    else
        echo "❌ 未检测到 RealSense 设备"
        echo ""
        echo "请检查："
        echo "  1. RealSense 相机是否已通过 USB 3.0 连接"
        echo "  2. USB 线缆是否为 USB 3.0（蓝色接口）"
        echo "  3. 设备供电是否正常"
        echo "  4. 尝试重新插拔 USB 线"
        echo ""
        read -p "是否继续尝试启动相机？(y/n): " -r
        if [[ ! $REPLY =~ ^[Yy]$ ]]; then
            echo "已取消启动"
            exit 0
        fi
    ...
    echo "⚠️  无法检测设备（rs-enumerate-devices 不可用）"
fi
...
LOG_FILE="$LOG_DIR/realsense_start_RGB_$TIMESTAMP.log"
echo "✅ 日志文件: $LOG_FILE"
...
# RGB 配置
echo -e "\n您选择启用 RGB 相机"
read -p "请输入 RGB 分辨率 (如 1280x720): " color_res
color_res=$(echo "$color_res" | tr -d ' ')
read -p "请输入 RGB 帧率(Hz): " color_fps

if [[ -z ${RGB_FPS[$color_res]} ]]; then
    echo "❌ 不支持的 RGB 分辨率！"
    exit 1
fi
if [[ ! " ${RGB_FPS[$color_res]} " =~ " ${color_fps} " ]]; then
    echo "❌ ${color_res} 不支持 FPS=${color_fps}"
    exit 1
fi

CMD_PARAMS="$CMD_PARAMS enable_color:=true \
    rgb_camera.color_profile:=${color_res}x${color_fps}"

# RGB 相机不需要点云，但可以启用其他功能
read -p $'\n是否启用对齐功能（将深度对齐到RGB）(y/n)? ' enable_align
if [[ "$enable_align" == "y" ]]; then
    CMD_PARAMS="$CMD_PARAMS align_depth.enable:=true"
else
    CMD_PARAMS="$CMD_PARAMS align_depth.enable:=false"
fi
...
echo "设备检测: $([ "$DEVICE_FOUND" = true ] && echo "设备已连接" || echo "设备未检测到")" >> "$LOG_FILE"

if [ "$DEVICE_FOUND" = true ]; then
    echo "设备信息:" >> "$LOG_FILE"
    echo "$DEVICE_INFO" >> "$LOG_FILE"
fi
echo "RGB 参数: ${color_res} @ ${color_fps} FPS" >> "$LOG_FILE"
echo "启用对齐: $([[ $enable_align == "y" ]] && echo Yes || echo No)" >> "$LOG_FILE"
echo "----------------------------------------" >> "$LOG_FILE"
