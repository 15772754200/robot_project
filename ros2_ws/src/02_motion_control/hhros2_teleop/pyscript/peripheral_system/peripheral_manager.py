...
LOG_DIR = Path("run_logs/peripheral_manager/")
LOG_DIR.mkdir(parents=True, exist_ok=True)
LOG_DIR.chmod(0o777)
...
# 手动设置日志文件权限为0o777
log_file_path = LOG_DIR / 'peripheral_manager.log'
...
class OperationLogger:
    ...
    def ensure_log_file(self):
        ...
        if not self.operation_log_file.exists():
            ...
            self.operation_log_file.chmod(0o777)
...
class PeripheralManager:
    ...
    def ensure_log_files(self):
        ...
        if not self.connection_log_file.exists():
            ...
            self.connection_log_file.chmod(0o777)
        if not self.enable_log_file.exists():
            self.enable_log_file.write_text('[]')
            self.enable_log_file.chmod(0o777)
...
class PeripheralManagerUI:
    ...
    def close_specific_terminal(self,unique_id):
        try:
            result = subprocess.run(['pgrep', '-f', f'periman终端管理器_{unique_id}'], capture_output=True, text=True, check=True)
...
def main():
    ...
    try:
        ...
        # 启动UI
        ui = PeripheralManagerUI(unique_id)
        ui.main_menu()
