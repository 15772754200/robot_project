...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(LOG_DIR, 'system_manager.log'), encoding='utf-8'),
        logging.StreamHandler()
    ]
)
# 手动设置日志文件权限为0o777
log_file_path = LOG_DIR / 'system_manager.log'
if log_file_path.exists():
    os.chmod(str(log_file_path), 0o777)
...
class SystemManagerUI:
    ...
    def main_menu(self):
        ...
        while self.running:
            ...
            if choice == '1':
                ...
                self.running = False
                print("\n感谢使用，再见！")
                print("uuid值为：" + self.unique_id)
                self.close_specific_terminal(self.unique_id)
            else:
                print("\n无效选择，请重试")
                time.sleep(1)
    ...
    def close_specific_terminal(self,unique_id):
        try:
            result = subprocess.run(['pgrep', '-f', f'syscon终端管理器_{unique_id}'], capture_output=True, text=True, check=True)
